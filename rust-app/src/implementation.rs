...
const ARG_ARRAY_SIZE: usize = 240;
...
) -> Option<()>
{
    ...
    match tx_type {
        0 | 1 => {
            write_scroller("Transfer", |w| Ok(write!(w
...
) -> Option<()>
{
    ...
    write_scroller("Paying Gas", |w| Ok(write!(w, "at most {} at price {}", from_utf8(gas_limit)?, from_utf8(gas_price)?)?))?;
    Some(())
}
...
type HasherAndPrivKey = (Hasher, cx_ecfp_256_private_key_s);

type TxParams1ParserT = (SubDefT, (DefaultInterp, (SubDefT, (SubDefT, (SubDefT, SubDefT)))));
const TX_PARAMS1_PARSER: TxParams1ParserT
    = (SUB_DEF, (DefaultInterp, (SUB_DEF, (SUB_DEF, (SUB_DEF, SUB_DEF)))));

pub type PathRecipientAmountT = impl InterpParser<MakeTransferTxParameters1, Returning = HasherAndPrivKey>;

const PATH_RECIPIENT_AMOUNT_PARSER: PathRecipientAmountT
  = MoveAction(
      TX_PARAMS1_PARSER
    , mkmvfn(|(path, optv1): <TxParams1ParserT as ParserCommon<MakeTransferTxParameters1>>::Returning
        ...
        loop {
            match state {
                MakeTxSubState::Init => {
                    ...
                    set_from_thunk(state, || MakeTxSubState::PathRecipientAmount(<PathRecipientAmountT as ParserCommon<MakeTransferTxParameters1>>::init(&PATH_RECIPIENT_AMOUNT_PARSER)))
                }
                MakeTxSubState::PathRecipientAmount(ref mut sub) => {
                    cursor = <PathRecipientAmountT as InterpParser<MakeTransferTxParameters1>>::parse(&PATH_RECIPIENT_AMOUNT_PARSER, sub, cursor, hasher_and_privkey)?;
